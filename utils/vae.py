...
class Sampling(layers.Layer):
    """
    Custom layer that samples from a normal distribution using the reparameterisation trick.

    Attributes:
        seed_generator (keras.random.SeedGenerator): Seed generator for reproducibility.

    Methods:
        call(inputs): Samples from the distribution defined by the inputs.
    """
    ...
    def call(self, inputs):
        """
        Samples from the distribution defined by the inputs.

        Parameters:
            inputs (tuple): A tuple containing the mean and log variance of the distribution.

        Returns:
            Tensor: A tensor representing the sampled points.
        """
...
class CustomVAE(keras.Model):
    ...
    Attributes:
        ...
        init (str): Weight initialiser to use.
    ...
    Methods:
        build_encoder(dims): Builds the encoder part of the VAE.
    """

    def __init__(
        self, base_dir="my_models/models", act="swish", init="glorot_uniform", **kwargs
    ):
        super().__init__(**kwargs)
        self.act = act
        self.init = init
    ...
    def build_encoder(self, dims):
        ...
        Parameters:
            dims (list): List of dimensions for each layer in the encoder.
    ...
    def build_decoder(self, dims):
        ...
        Parameters:
            dims (list): List of dimensions for each layer in the decoder, ordered from the output to the latent space.
    ...
    def train_step(self, data):
        """
        VAE training step with a custom loss function.

        Parameters:
            data (tuple): A tuple containing the input data and the target data.

        Returns:
            dict: A dictionary containing the current loss and metrics.
        """
    ...
    def build_autoencoder(self, dims, act=None, init=None):
        ...
        Parameters:
            dims (list): list of number of units in each layer of encoder. dims[0] is input dim, dims[-1] is units in hidden layer. The decoder is symmetric with encoder. So number of layers of the auto-encoder is 2*len(dims)-1
            act (str): activation, not applied to Input, Hidden and Output layers
            init (str): initialisation, not applied to Input, Hidden and Output layers
    ...
    def train(
        self, df, to_forecast, lookback, out_steps, latent_dim=5, train_again=False
    ):
        """
        ...
        Parameters:
            ...
            to_forecast (str): The column to forecast.
            lookback (int): The size of the memory window.
            out_steps (int): The number of steps to forecast.
            latent_dim (int): The dimension of the latent space.
            train_again (bool): Whether to train the model again or load it from disk for inference.

        Returns:
            pd.DataFrame: The reduced data.
        """
        columns_to_keep = [
    ...
    def predict(self, df):
        ...
        reduced_df = pd.DataFrame(
            self.encoder.predict(df_reduce)[-1],
            columns=["Z{}".format(i) for i in range(self.latent_dim)],
        )
        return pd.concat([df_keep, reduced_df], axis=1)
